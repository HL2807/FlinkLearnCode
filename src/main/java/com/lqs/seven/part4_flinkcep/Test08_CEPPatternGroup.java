...
/**
 * @Author lqs
 * @Date 2022年04月06日 17:28:16
 * @Version 1.0.0
 * @ClassName Test08_CEPPatternGroup
 * @Describe 模式组
 * 在前面的代码中次数只能用在某个模式上, 比如: .begin(...).where(...).next(...).where(...).times(2)
 * 这里的次数只会用在next这个模式上, 而不会用在begin模式上.
 * 如果需要用在多个模式上,可以使用模式组!
 */
public class Test08_CEPPatternGroup {
